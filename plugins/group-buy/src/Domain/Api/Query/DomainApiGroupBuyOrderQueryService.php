...
final class DomainApiGroupBuyOrderQueryService
{
    ...
    public function activitySummaryPage(array $filters, int $page, int $pageSize): array
    {
        ...
        $originalCounts = $this->countOriginalBuyers($activityIds, $skuIds, $activities);
    ...
    /**
     * 统计原价购买人数：order_type=group_buy 且不在 group_buy_orders 表中的订单.
     */
    private function countOriginalBuyers(array $activityIds, array $skuIds, $activities): array
    {
        if (empty($skuIds)) {
            return [];
        }
        ...
        // 通过 OrderItem 模型查询，关联 order 筛选条件，避免原始 SQL
