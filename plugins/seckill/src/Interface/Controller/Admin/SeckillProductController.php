...
final class SeckillProductController extends AbstractController
{
    ...
    {
        return $this->success(array_map(static fn ($p) => $p->toArray(), $this->queryService->findBySessionId($sessionId)));
    }
    ...
    public function show(int $id): Result
    {
        ...
        return $product ? $this->success($product->toArray()) : $this->error('商品不存在', 404);
    }
    ...
    public function batchStore(SeckillProductRequest $request): Result
    {
        ...
        $this->commandService->batchCreate((int) $data['activity_id'], (int) $data['session_id'], $data['products']);
