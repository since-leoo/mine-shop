...
use Plugin\Since\Seckill\Domain\ValueObject\SessionPeriod;
use Plugin\Since\Seckill\Domain\ValueObject\SessionRules;
use Carbon\Carbon;
...
final class SeckillSessionEntity
{
    ...
    public static function reconstitute(
        int $id, int $activityId, string $startTime, string $endTime, string $status,
        int $maxQuantityPerUser, int $totalQuantity, int $soldQuantity, int $sortOrder,
        bool $isEnabled, ?array $rulesData, ?string $remark,
        ?Carbon $createdAt = null, ?Carbon $updatedAt = null
    ): self {
        $entity = new self();
        $entity->id = $id; $entity->activityId = $activityId;
        ...
        $entity->sortOrder = $sortOrder; $entity->isEnabled = $isEnabled;
        $entity->remark = $remark; $entity->createdAt = $createdAt; $entity->updatedAt = $updatedAt;
        return $entity;
    }
    ...
    public function update(SeckillSessionInput $dto): self
    {
        if ($dto->getStartTime() && $dto->getEndTime()) {
            ...
            if (!$newPeriod->equals($this->period)) { $this->period = $newPeriod; }
        }
        ...
        if ($dto->getStatus()) { $this->status = SeckillStatus::from($dto->getStatus()); }
        return $this;
    }
    ...
    public function getPeriod(): SessionPeriod { return $this->period; }
    public function getStatus(): SeckillStatus { return $this->status; }
    public function getRules(): SessionRules { return $this->rules; }
    ...
    public function canPurchase(): bool { return $this->isEnabled && !$this->stock->isSoldOut() && $this->period->isActive(); }
    public function canBeEdited(): bool { return $this->status !== SeckillStatus::ACTIVE && $this->status !== SeckillStatus::ENDED; }
    public function canBeDeleted(): bool { return $this->status !== SeckillStatus::ACTIVE && $this->stock->getSoldQuantity() <= 0; }

    public function toggleEnabled(): self { $this->isEnabled = !$this->isEnabled; return $this; }

    public function start(): self
    {
        if ($this->status !== SeckillStatus::PENDING) { throw new \DomainException('只有待开始的场次才能启动'); }
        if (!$this->isEnabled) { throw new \DomainException('未启用的场次不能启动'); }
    ...
    {
        if ($this->status === SeckillStatus::ENDED) { throw new \DomainException('场次已经结束'); }
        $this->status = SeckillStatus::ENDED;
        return $this;
    }

    public function soldOut(): self { $this->status = SeckillStatus::SOLD_OUT; return $this; }

    public function sell(int $quantity): self
    {
        $this->stock = $this->stock->sell($quantity);
        if ($this->stock->isSoldOut()) { $this->status = SeckillStatus::SOLD_OUT; }
        return $this;
    }
