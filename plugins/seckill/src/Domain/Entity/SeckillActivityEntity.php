...
use Plugin\Since\Seckill\Domain\ValueObject\ActivityRules;
use Carbon\Carbon;
...
final class SeckillActivityEntity
{
    ...
    public static function reconstitute(
        int $id, string $title, ?string $description, string $status, bool $isEnabled,
        ?array $rulesData, ?string $remark, ?Carbon $createdAt = null, ?Carbon $updatedAt = null
    ): self {
        $entity = new self();
        $entity->id = $id;
        $entity->title = $title;
    ...
    {
        if ($dto->getTitle()) { $this->title = $dto->getTitle(); }
        if ($dto->getDescription() !== null) { $this->description = $dto->getDescription(); }
        if ($dto->getRules()) { $this->rules = new ActivityRules($dto->getRules()); }
    ...
    public function getRules(): ActivityRules { return $this->rules; }
    ...
    public function canBeDeleted(): bool { return $this->status !== SeckillStatus::ACTIVE; }
    ...
    {
        if (!$this->canBeCancelled()) { throw new \DomainException('当前活动状态不允许取消'); }
    ...
    {
        if ($this->status !== SeckillStatus::PENDING) { throw new \DomainException('只有待开始的活动才能启动'); }
        if (!$this->isEnabled) { throw new \DomainException('未启用的活动不能启动'); }
    ...
    {
        if ($this->status === SeckillStatus::ENDED) { throw new \DomainException('活动已经结束'); }
