...
final class DomainSeckillSessionService extends IService
{
    ...
    public function findByActivityId(int $activityId): array { return $this->repository->findByActivityId($activityId); }
    ...
    public function update(SeckillSessionInput $dto): bool
    {
        ...
        if (!$session) { throw new \RuntimeException('场次不存在'); }
        $entity = SeckillSessionMapper::fromModel($session);
        if (!$entity->canBeEdited()) { throw new \DomainException('当前场次状态不允许编辑'); }
        $entity->update($dto);
    ...
    public function updateQuantityStats(int $sessionId): void { $this->repository->updateQuantityStats($sessionId); }

    public function start(int $id): bool
    {
        $session = $this->repository->findById($id);
        if (!$session) { throw new \RuntimeException("场次不存在: ID={$id}"); }
        $entity = SeckillSessionMapper::fromModel($session);
        $entity->start();
        return $this->repository->updateFromEntity($entity);
    }
