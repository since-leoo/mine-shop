...
final class SeckillCacheService
{
    ...
    public function warmSession(int $sessionId): void
    {
        ...
        if ($fields !== []) { $this->cache->hMset($productsKey, $fields); }
    ...
    private function warmStockHash(int $sessionId, array $products): void
    {
        ...
        if ($stockFields !== []) { $this->cache->hMset($hashKey, $stockFields); }
    }
    ...
    public function getSession(int $sessionId): ?SeckillSessionEntity
    {
        ...
        if (\is_string($json) && $json !== '') {
            ...
            if (\is_array($data)) { return $this->hydrateSession($data); }
        }
        $model = $this->sessionRepository->findById($sessionId);
    ...
    public function getProductBySkuId(int $sessionId, int $skuId): ?SeckillProductEntity
    {
        ...
        if (\is_string($json) && $json !== '') {
            $data = json_decode($json, true);
            if (\is_array($data)) { return $this->hydrateProduct($data); }
        }
    ...
    private function hydrateSession(array $data): SeckillSessionEntity
    {
        return SeckillSessionEntity::reconstitute(
            id: (int) ($data['id'] ?? 0), activityId: (int) ($data['activity_id'] ?? 0),
            startTime: (string) ($data['start_time'] ?? ''), endTime: (string) ($data['end_time'] ?? ''),
            status: (string) ($data['status'] ?? 'pending'), maxQuantityPerUser: (int) ($data['max_quantity_per_user'] ?? 1),
            totalQuantity: (int) ($data['total_quantity'] ?? 0), soldQuantity: (int) ($data['sold_quantity'] ?? 0),
            sortOrder: (int) ($data['sort_order'] ?? 0), isEnabled: (bool) ($data['is_enabled'] ?? false),
            rulesData: \is_array($data['rules'] ?? null) ? $data['rules'] : null, remark: $data['remark'] ?? null,
        );
    }
    ...
    private function hydrateProduct(array $data): SeckillProductEntity
    {
        return SeckillProductEntity::reconstitute(
            id: (int) ($data['id'] ?? 0), activityId: (int) ($data['activity_id'] ?? 0),
            sessionId: (int) ($data['session_id'] ?? 0), productId: (int) ($data['product_id'] ?? 0),
            productSkuId: (int) ($data['product_sku_id'] ?? 0),
            originalPrice: (int) ($data['original_price'] ?? 0), seckillPrice: (int) ($data['seckill_price'] ?? 0),
            quantity: (int) ($data['quantity'] ?? 0), soldQuantity: (int) ($data['sold_quantity'] ?? 0),
            maxQuantityPerUser: (int) ($data['max_quantity_per_user'] ?? 1),
            sortOrder: (int) ($data['sort_order'] ?? 0), isEnabled: (bool) ($data['is_enabled'] ?? false),
        );
    }

    private function sessionKey(int $id): string { return \sprintf('session:%d', $id); }
    private function sessionProductsKey(int $id): string { return \sprintf('session:%d:products', $id); }
}
