...
final class DiyPageController extends AbstractController
{
    public function __construct(
        ...
        private readonly AppDiyTemplateCommandService $templateCommandService,
    ...
    #[PostMapping(path: '')]
    #[Permission(code: 'mall:diy:page:create')]
    ...
    #[Permission(code: 'mall:diy:page:enable')]
    ...
    #[PostMapping(path: '{id:\d+}/save-as-template')]
    #[Permission(code: 'mall:diy:template:create')]
    public function saveAsTemplate(int $id, DiyTemplateRequest $request): Result
    {
        $template = $this->templateCommandService->savePageAsTemplate($id, $request->toDto(), $this->currentUser->id());

        return $this->success($template->toArray(), '保存为装修模板成功', 201);
    }
}
