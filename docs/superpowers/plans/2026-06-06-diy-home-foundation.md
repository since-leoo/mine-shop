...
**目标：** 建立第一期可落地的首页 DIY 装修闭环，包括后台草稿/发布、公开接口、小程序渲染器和实用型后台编辑器。

**架构：** 新增 `Content/DiyPage` 领域，用版本化 JSON 页面结构保存装修页面。后台接口负责草稿与发布流程，公开接口只返回已发布页面结构，并在服务端补齐商品组数据。小程序通过类型化组件注册表渲染已发布页面结构，同时保留现有首页作为兜底。
...
- 新建 `app/Domain/Content/DiyPage/Service/DomainDiyPageService.php`：草稿、发布、重置、查询规则。
...
- 新建 `web/src/modules/mall/views/diy/home/index.vue`：编辑器外壳。
...
创建 `diy_pages`，字段包括 `page_key`、`title`、`platform`、`status`、`published_version_id`、作者字段、时间戳和软删除字段。

创建 `diy_page_versions`，字段包括 `page_id`、`version_no`、`status`、`schema`、`published_at`、作者字段和时间戳。

- [ ] **步骤 2：添加模型类**

`DiyPage` 使用 `SoftDeletes`，表名为 `diy_pages`，声明可填充字段、类型转换，以及 `versions()`、`publishedVersion()` 关联。
...
- `saveDraft()` 在页面不存在时创建页面，并创建或更新草稿版本。
...
领域服务负责调用 `DiyPageSchemaVo` 校验页面结构，提供默认首页页面结构，并处理草稿、发布、重置和查询规则。
...
`AppDiyPageCommandService` 对 `saveDraft()`、`publish()`、`resetDraft()` 使用 `Db::transaction()`。
...
使用 `DomainDiyPageService::getPublished($pageKey)`。对 `product-group` 组件调用 `AppApiProductQueryService` 补齐商品：
...
路由：`GET /api/v1/diy/pages/{pageKey}`，并使用 `ApiSignatureMiddleware`。
...
`fetchDiyPage(pageKey: string)` 通过现有 `request()` 请求 `/api/v1/diy/pages/${pageKey}`。
...
在现有加载流程中调用 `fetchDiyPage('home')`。如果返回 `components.length > 0`，渲染 `DiyRenderer`；否则走当前 `fetchHome()` 逻辑。
...
**文件：**
- 新建：`web/src/modules/mall/views/diy/home/index.vue`
...
git commit -m "feat(web): 添加首页装修编辑器"
```
...
新增菜单：`商城管理 / 页面装修 / 首页装修`，组件路径为 `mall/views/diy/home/index`。
...
调用 `GET /api/v1/diy/pages/home`，并带上必要接口签名请求头。确认返回已发布页面结构。
